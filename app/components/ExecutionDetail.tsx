...
export function ExecutionDetail({
  ...
  useEffect(() => {
    if (TERMINAL_STATUSES.includes(initialExecution.status)) {
      return;
    }

    const source = new EventSource(`/api/executions/${initialExecution.id}/stream`);

    source.addEventListener("execution", (event) => {
      const updated = JSON.parse(event.data) as Execution;
    ...
    source.addEventListener("log", (event) => {
      const line = JSON.parse(event.data) as ExecutionLog;
      setLogs((prev) => [...prev, line]);
    });

    return () => source.close();
  }, [initialExecution.id, initialExecution.status]);
    ...
    <div className="flex max-w-3xl flex-col gap-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {RETRYABLE_STATUSES.includes(execution.status) && <RetryExecutionButton action={retryAction} />}
          {CANCELLABLE_STATUSES.includes(execution.status) && <CancelExecutionButton action={cancelAction} />}
        </div>
      </div>
